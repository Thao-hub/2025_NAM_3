...
COMPOUND TRIGGER
    ...
    PROCEDURE add_mahd(p_mahd HoaDon.MaHD%TYPE) IS
        v_found BOOLEAN := FALSE;
    BEGIN
        FOR i IN 1 .. g_mahd_list.COUNT LOOP
            IF g_mahd_list(i) = p_mahd THEN
                v_found := TRUE;
                EXIT;
            END IF;
        END LOOP;
        ...
        IF NOT v_found THEN
            g_mahd_list(g_mahd_list.COUNT + 1) := p_mahd;
        END IF;
    END add_mahd;
    ...
    BEGIN
        ...
        IF NVL(:NEW.SoTien, 0) <= 0 OR :NEW.SoTien >= 50000000 THEN
            RAISE_APPLICATION_ERROR(-20122, 'SoTien phai > 0 va < 50000000');
        END IF;
