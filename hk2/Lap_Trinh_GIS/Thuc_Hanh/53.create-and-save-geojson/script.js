...
exportJSON.addEventListener("click", () => {
	...
	const convertedData = `text/json;charset=utf-8,${encodeURIComponent(
		JSON.stringify(data)
	)}`;
...
const localStorageRaw = localStorage.getItem("geojson");
const geojsonFromLocalStorage = localStorageRaw
...
function setGeojsonToMap(geojson) {
	...
	const feature = L.geoJSON(geojson, {
		...
		pointToLayer: (geoFeature, latLng) => {
			...
			if (geoFeature.properties.type === "circlemarker") {
		...
		onEachFeature: (geoFeature, layer) => {
			drawnItems.addLayer(layer);
			const coordinates = geoFeature.geometry.coordinates.toString();
...
function openFile(event) {
	...
	reader.onload = () => {
		try {
			...
			Notiflix.Notify.failure("Invalid GeoJSON file");
		}
	};
